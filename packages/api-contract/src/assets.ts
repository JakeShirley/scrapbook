...
export const assetResponseSchema = z
  .object({
    ...
    dateTaken: z.string().datetime().nullable().openapi({ example: "2026-04-12T15:30:00.000Z" }),
    cameraMake: z.string().nullable().openapi({ example: "Apple" }),
    cameraModel: z.string().nullable().openapi({ example: "iPhone 15 Pro" }),
    lensModel: z.string().nullable().openapi({ example: "iPhone 15 Pro back triple camera" }),
    isoSpeed: z.number().int().nonnegative().nullable().openapi({ example: 100 }),
    fNumber: z.number().nonnegative().nullable().openapi({ example: 1.78 }),
    exposureTimeSeconds: z.number().nonnegative().nullable().openapi({ example: 0.004 }),
    focalLengthMm: z.number().nonnegative().nullable().openapi({ example: 6.86 }),
    focalLength35mmMm: z.number().int().nonnegative().nullable().openapi({ example: 24 }),
    orientation: z.number().int().nullable().openapi({ example: 1 }),
    gpsLatitude: z.number().nullable().openapi({ example: 47.6097 }),
    gpsLongitude: z.number().nullable().openapi({ example: -122.3331 }),
    gpsAltitudeMeters: z.number().nullable().openapi({ example: 56.3 }),
    createdAt: z.string().datetime().openapi({ example: "2026-05-17T12:00:00.000Z" }),
    updatedAt: z.string().datetime().openapi({ example: "2026-05-17T12:00:00.000Z" }),
  })
