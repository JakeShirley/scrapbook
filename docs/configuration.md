...
| `SCRAPBOOK_DATA_DIR` | `./storage/dev` | No | No | Root for local SQLite files, page document JSON files, uploads, variants, previews, and exports. |

The Docker Compose API service overrides `API_HOST` to `0.0.0.0`, `WEB_ORIGIN` to `http://localhost:4000`, and `SCRAPBOOK_DATA_DIR` to `/data/scrapbook`. That container path is backed by the `scrapbook-data` Docker volume so SQLite files, page documents, uploads, variants, previews, and exports persist across container restarts.
